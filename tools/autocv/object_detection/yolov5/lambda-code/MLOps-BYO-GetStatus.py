...
print('importing sagemaker')
import sagemaker as sage
print('imported sagemaker')

sagemaker = boto3.client('sagemaker')
code_pipeline = boto3.client('codepipeline')
panorama = boto3.client('panorama')
sqs_client = boto3.client("sqs")

print('Panorama Devices')
print(panorama.list_devices())
    ...
    try:
        ...
        if stage == 'Training':
            ...
            if status == 'Completed':
                ...
                # Send name and model_data_url to convert-onnx-queue
                print('Sending name and model_data_url to convert-onnx-queue for conversion to ONNX')
                queue_url = sqs_client.get_queue_url(QueueName='convert-onnx-queue')
                message = 'job_name: {}, model_data: {}'.format(name, model_data_url)
                sqs_client.send_message(QueueUrl=queue_url["QueueUrl"],MessageBody=json.dumps(message))
                
                # query succes from optimize-model-queue, if success is there then we write success
