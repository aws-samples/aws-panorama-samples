...
def detector_to_simple_pose(img, class_ids, scores, bounding_boxs,
                            output_shape=(256, 192), scale=1.25, person_index=0,
...
def reset_tracker():
    ...
    """
    xpart_tracker = {'anks': np.array([]), 'shdr': np.array([]),'anks-shdr': np.array([])}
    ypart_tracker = {'anks': np.array([]), 'shdr': np.array([]),'anks-shdr': np.array([])}
